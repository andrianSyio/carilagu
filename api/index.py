...
# Secara eksplisit menunjuk ke lokasi FFmpeg & FFprobe di server Vercel
# yang diinstal oleh apt-get.
...
def identify_from_file():
    ...
    try:
        if uploaded_file.content_type.startswith('video/'):
            ...
            buf = BytesIO(); video.export(buf, format='wav'); audio_sample = buf.getvalue()
        else:
...
def identify_from_url():
    ...
    if not url: return jsonify({'success': False, 'message': 'URL tidak ditemukan.'}), 400
    try:
        headers = {'User-Agent': 'Mozilla/5.0'}; response = requests.head(url, allow_redirects=True, timeout=10, headers=headers); expanded_url = response.url
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl: ydl.download([expanded_url])
        with open(temp_filename, 'rb') as f: audio_sample = f.read()
        result_acr = identify_audio_sample(audio_sample)
    finally:
