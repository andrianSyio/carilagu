...
def identify_from_url():
    ...
    # --- BLOK KODE BARU UNTUK MENANGANI URL PENDEK ---
    try:
        # Jika ini URL pendek, 'requests' akan otomatis mengikuti redirect dan memberikan URL final
        # Kita gunakan HEAD request agar lebih cepat, tidak perlu download konten halaman
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}
        response = requests.head(url, allow_redirects=True, timeout=10, headers=headers)
        expanded_url = response.url # Ini adalah URL panjangnya
    ...
    try:
        # Gunakan URL panjang yang sudah di-ekspansi
    ...
    if status.get('msg') == 'Success':
        ...
        return jsonify({'success': False, 'message': 'Lagu tidak dapat diidentifikasi dari URL ini.'}), 404def identify_audio_sample(audio_sample):
